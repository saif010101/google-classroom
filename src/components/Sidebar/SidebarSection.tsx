    ...
    return (
        <>
            ...
            <ul className="w-full flex flex-col ">
                {data?.map(course => (
                    <NavLink onClick={handleClick} to={`/c/${course.class_code}/stream`} key={course.class_code} className={({ isActive }) => `px-6 py-2 rounded-full hover:bg-gray-200 cursor-pointer transition duration-100 ease-in ${isActive && 'bg-gray-300'}`}>
